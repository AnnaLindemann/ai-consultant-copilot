...
- `WorkspaceRepository`, `UserRepository`, `ClientInvitationRepository`, `NotificationRepository`, `AuditTrailRepository` — persistence for the access and collaboration side (Phase 3A). `AuditTrailRepository` is **append-only**: it exposes appending and reading, never update or delete.
...
- **The Audit Trail is append-only**, like Consultant Report versions and the Technology Update History; entries are never rewritten or deleted.
...
- **Access-side tables (Phase 3A).** `Workspace`, `User` (with role and workspace), `ClientInvitation` (engagement-scoped, time-bounded, revocable), `Notification`, and `AuditTrail` are their own table group. The audit table is **append-only** — no update or delete path exists to it in code — mirroring the Technology Update History and report versions.
...
This section is the implementation shape of domain-model §3A. It is delivered by roadmap **Phase 3A** — **not** Phase 11, which now covers deployment and operations only.
...
- **Append-only, by construction.** The repository exposes append and read only; there is no update or delete path. This mirrors report versions and the Technology Update History.
...
- **Hardening deferred, not ignored.** Production-grade error observability and recovery are explicitly a Phase 11 concern; earlier phases keep the structured-outcome discipline above so the hardening phase has a consistent base to build on. Note that the *access-control model itself* is not deferred — it is Phase 3A; Phase 11 verifies it holds in the production environment.
...
- **Production readiness (Phase 11)** — deployment, monitoring, operational security, backup, recovery, performance — layers around the stateless backend and existing observability without touching the domain. It **operates and verifies** the Phase 3A access model rather than introducing it.
...
- **Single-consultant until Phase 3A, workspace-partitioned multi-user from Phase 3A** *(revised in 1.2; previously deferred to Phase 11)*. The pre-Phase 3A implementation path remains single-consultant, with the backend kept stateless so multi-user is addable. From Phase 3A, authentication, authorization, roles, workspace isolation, and engagement ownership are in place, and every later phase is built inside that boundary. Phase 11 no longer owns access control; it deploys, monitors, and hardens it.
