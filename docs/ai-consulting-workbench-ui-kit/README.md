# AI Consulting Workbench UI Kit

This folder is a frontend design source for the project.

## Files

- `UI-KIT.md` — complete design, navigation, page, interaction, accessibility, security, and implementation specification.
- `design-tokens.json` — machine-readable colors, typography, spacing, radius, layout, and breakpoints.
- `role-permissions.csv` — role capability matrix.
- `screen-inventory.csv` — full Client, Manager, and Administrator page inventory.

## Important scope note

The approved product and frozen project documents now include a Client Portal that supports Discovery and manager-published read-only proposals. The Client receives an e-mail notification and may open or download the published PDF.

Before implementing proposal delivery, deliberately update the relevant frozen product/domain/roadmap/architecture documents and authorization contract. Do not treat this UI document alone as an authorization source.

## Suggested repository location

```text
docs/ai-consulting-workbench-ui-kit/
  README.md
  UI-KIT.md
  design-tokens.json
  role-permissions.csv
  screen-inventory.csv
```
